...
1. **Privacy (GDPR):** NCE already carries the data-layer machinery (RLS tenancy, `me_app` DSAR/GDPR surface, C9b no-person-grain guard). Copper's obligations: no personal data in URLs or client logs, DSAR surface wired into the shell (C lane), EU-resident hosting assumption for the BFF, and no third-party scripts/CDNs in the product app (the docs site's CDN use is internal-only).
...
5. **NIS2 posture:** Copper/BFF follow the security-hygiene expectations of an operator in scope via its customers: authenticated everything, audit trails on world-writes (NCE event log), no default credentials, dependency scanning in CI. A formal NIS2 mapping is a documentation wave when the netops surfaces arrive.

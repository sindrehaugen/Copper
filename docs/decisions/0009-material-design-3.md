...
2. **Color schemes are generated, not hand-picked:** `@material/material-color-utilities` (Google's official library, Apache-2.0 — passes the licence gate) generates the full M3 tonal palettes and color roles from Copper's brand seed **`#6750A4` (M3 baseline)**. Both light and dark scheme token sets are emitted at build time.

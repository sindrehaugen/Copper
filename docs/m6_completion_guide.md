...
> **Status:** handoff · **Date:** 2026-08-28 · **From:** Sindre (via the Copper planning session) · **To:** the ML orchestrator, to be pushed UP FRONT in the queue
...
- **Contract A:** all five node types are `system_design`-owned in `node-ownership.json` — no registry edits needed for W13–W18. **BOM_LINE is explicitly OUT of this guide's scope** (that is B132a/133b + debt items 7/19/20; a separate coordination with Copper comes later — do not let any wave here touch it).

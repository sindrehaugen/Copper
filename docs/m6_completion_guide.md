...
| `system_design_delete_planned` (W17) | cacheable=False, admin_only=True, mutation=True | `DELETE /api/system-design/planned` | new (W17) |
...
**Shipped Deviations (Copper absorbed):**
1. `system_design_delete_planned` shipped with `admin_only=True`.
2. `system_design_delete_planned` defaults to soft retire; permanent deletion requires `permanent: true` + `actor`.

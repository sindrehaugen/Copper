...
// 1. Find Layout function boundaries
const layoutStart = code.indexOf("function Layout({ children }: { children: ReactNode }) {");
let layoutEnd = code.indexOf("export function ConnectedCanvasView() {");
code = code.substring(0, layoutStart) + layoutCode + "\n\n" + code.substring(layoutEnd);

// 2. Find AppShell function boundaries
const appShellStart = code.indexOf("export function AppShell() {");
code = code.substring(0, appShellStart) + appShellCode;

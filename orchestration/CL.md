...
* [PASSED TAG] B38 — V.W3 rack-fit: u_height vs gap, is_full_depth collisions · tier: T2 · dep: B41 (transitively held via B41→B15 `[ADOPTED - ML]`) [NO TAG]
...
* [PASSED TAG] B59 — B.W1 bom-contract: node-ownership row + label scheme (no `_`/`%` — LIKE-wildcard bug class) + status-edge writer design — **coordinate with the ML orchestrator (NCE ML-B132a/133b territory) before any dispatch** · tier: T3 · dep: B35 [NO TAG]
* [PASSED TAG] B60 — B.W2 bom-emit: design→BOM_LINE emission with per-line provenance (design-generated origination path); delta emission on revision · tier: T3 · dep: B59 [NO TAG]
...
* [PASSED TAG] B72 — C.W1 dsar-surface: wire NCE `me_app` DSAR/GDPR endpoints into the shell · tier: T2 · dep: B68b,B70 [NO TAG]

...
* [WAITING TAG] B5 — F.W5 es-reader: EasySchematic **format** reader (clean PORT-list port of `fraEasySchematic.js`) → schema · tier: T2 · dep: B4b [NO TAG]
...
* [RUNNING] B19 — P.W1 bff-scaffold: `bff/` Hono TS server; config seams (`NCE_BASE_URL`, `NCE_API_KEY` via env/file — key class + blast radius per the seam audit; dev-identity seam per ADR-0011), `/healthz`, session stub (replaced by B75) · tier: T2 · dep: B1 [NO TAG]

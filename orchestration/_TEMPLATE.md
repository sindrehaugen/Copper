...
6. **Licence firewall (voids the wave if broken):** never open, quote, or port from `C:\Claude\EasySchematic\**` or the three AGPL-derived steps-ai files named in `docs/decisions/0005-…md` §FORBIDDEN. If this brief's task seems to need them, STOP and report. Any dependency you are told to add must be MIT/BSD/Apache/ISC/CC0 — check its licence field before installing.

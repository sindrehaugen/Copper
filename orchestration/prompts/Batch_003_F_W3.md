...
**Goal:** the single source of drawing geometry. One exported `PITCH = 24` (px). Card geometry derived so **ports land exactly on grid lines**: `CARD_WIDTH`, `CARD_HEADER_H`, `CARD_PAD_Y`, `PORT_ROW_H` — every exported dimension an integer multiple of `PITCH`, **with `PORT_ROW_H = 2 × PITCH` (required: the first-port-center invariant below contains `PORT_ROW_H / 2`, so an odd multiple would break it — this is arithmetic, not taste)**, plus the derived invariant that the first port dot's center offset (`CARD_HEADER_H + CARD_PAD_Y + PORT_ROW_H / 2`) is itself a multiple of `PITCH`. One `PORT_DOT_INSET` constant — the ONLY definition of "distance from card edge to port dot" any renderer may use; it is deliberately sub-pitch, so it ships as the sole pre-seeded entry in `NON_GRID_EXPORTS`. A pure `portDotPosition(cardX, cardY, portIndex, side)` helper returning grid-aligned coordinates (the dot center, before inset rendering).
...
2. Author the ratchet test: programmatically iterate **every numeric export** of the module (`Object.entries`) asserting `value % PITCH === 0` OR the export is explicitly listed in `NON_GRID_EXPORTS` (ships with exactly one entry: `PORT_DOT_INSET`; additions require a brief that justifies them). Assert `PORT_ROW_H === 2 * PITCH`, the first-port-center invariant, and that `portDotPosition` returns multiples of `PITCH` for indices 0–7 on both sides.

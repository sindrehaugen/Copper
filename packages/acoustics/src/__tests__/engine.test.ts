...
const db: EquipmentDatabase = {
  speakers: {
    '8R': { slug: '8R', manufacturer: 'T', model: '8Ω', impedance: 8, wattage_rms: 100, z_min: 6.4 },
  },
  cables: {
    '2.5': { slug: '2.5', manufacturer: 'T', model: '2.5mm²', resistance: 7.41, inductance: 0.65, capacitance: 110 },
  },
  amplifiers: {
    'amp': {
      slug: 'amp', manufacturer: 'T', model: 'X',
...
function makeNode(overrides: Partial<SignalNode> = {}): SignalNode {
  return {
    slug: 'L-1', parentId: null, userLabel: '',
...
describe('analyseChain', () => {
  ...
  const ampRack = { 'A-1': { slug: 'A-1', modelId: 'amp', channelsUsed: [1] } }
  ...
  it('flags Error when cable is grossly undersized', () => {
    const longCableDb: EquipmentDatabase = {
      ...db,
      cables: {
        'thin': { slug: 'thin', manufacturer: 'T', model: '0.2mm²', resistance: 100, inductance: 1, capacitance: 200 },
      },
    }
  ...
  it('supports recursive children (daisy chain)', () => {
    const child = makeNode({ slug: 'L-1.1', parentId: 'L-1', length: 5, ampInstanceId: '', ampChannel: null })
